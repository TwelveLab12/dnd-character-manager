 ...
 * ouvre le panneau de détail de l'arme (docs/adr/0042). `ready` : armes prêtes à dégainer, en
 * bordure pointillée et teinte neutre, pour les distinguer des armes en main (docs/adr/0054). */
    ...
    <ul aria-label={label} className="grid gap-2 sm:grid-cols-2">
      {attacks.map((attack) => {
        ...
        return (
          <li
            key={attack.itemId}
            className={`relative flex items-center gap-3 rounded-xl border px-3 py-2.5 sm:rounded-2xl sm:px-3.5 ${
              ready ? "border-foreground/25 border-dashed" : "bg-card"
            }`}
          >
            <button
              type="button"
            ...
            <span
              className={`grid size-8 shrink-0 place-items-center rounded-lg sm:size-9 ${
                ready ? "bg-accent/15 text-accent" : "bg-primary/10 text-primary"
              }`}
            >
            ...
            <span
              ...
              className={`font-heading text-[22px] font-bold tabular-nums sm:text-2xl ${
